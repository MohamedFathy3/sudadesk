...
export interface FormField {
  name: string;
  label: string;
  type: 'text' | 'number' | 'select' | 'email' | 'class-selector' | 'password' | 'date'| 'custom' |'datetime-local' |'custom-time' | 'time' | 'image' | 'switch' | 'textarea' | 'file' | 'tel' | 'url' | 'checkbox';
  required?: boolean;
...
onSave: (e: SaveOptions) => void;
  ...
  language?: Language; // إضافة دعم اللغة
}
...
// تحديث DataTableProps لدعم الترجمات
...
// إضافة ExtendedDataTableProps إذا كان مطلوباً
export interface ExtendedDataTableProps<T extends Entity> extends DataTableProps {
  data: T[];
  translations?: {
    actions: string;
    status: string;
...
// إضافة PaginationProps إذا لم يكن موجوداً
